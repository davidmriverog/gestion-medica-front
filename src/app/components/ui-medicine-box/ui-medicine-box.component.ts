...
import { MedicineFormComponent } from '../../modules/clinic/medicines/medicine-form.component';
import { MedicineService } from 'src/app/project/services/clinic/medicine.service';
import { MedicineModel } from 'src/app/project/models/clinic/medicine.model';

@Component({
  selector: 'ui-medicine-box',
  templateUrl: './ui-medicine-box.component.html',
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => UIMedicineBoxComponent),
      multi: true
    }
  ],
  encapsulation: ViewEncapsulation.None
})
export class UIMedicineBoxComponent implements OnInit, OnDestroy, ControlValueAccessor {
  ...
  search(event: { query: string }): void {
    ...
    this.filteredResult$ = this.medicineService.searching(event.query.toLocaleLowerCase());
  }
  ...
  populatedModalSelection(row: MedicineModel): void {
    this.isSeleted = true;
  ...
  create(): void {
    const formModal = this.modalService.open(MedicineFormComponent, <NgbModalOptions>{
      size: 'lg',
      centered: true
    });
    ...
    crudForm.onResult.subscribe((results: any) => {
      ...
      let currentResult: MedicineModel = results.data;
